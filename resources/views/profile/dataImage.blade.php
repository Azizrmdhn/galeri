...
@if ($dataImage->count() > 0)
    ...
    <div class="max-w-xs container bg-white rounded-xl shadow-lg transform transition duration-500 hover:scale-105 hover:shadow-2xl">
      ...
      <div>
        <div class="flex justify-center items-center gap-2">
            <span class="text-white text-xs font-bold rounded-lg bg-lime-500 inline-block  py-1.5 px-4 cursor-pointer">{{$image->categoryName}}</span>
            <a href="{{ url('detailImageEdit/'.$image->fotoID. '/'. $image->id) }}">
            <span class="text-white text-xs flex font-bold rounded-lg bg-lime-500 gap-2 py-1.5 px-4 cursor-pointer">
            ...
            <form  action="{{ route('dataImage.delete', $image->fotoID) }}" method="POST">
                ...
                @method('DELETE')
                <button type="submit">
                    <span class="text-white text-xs font-bold gap-1 rounded-lg flex bg-gray-500  gap-2 py-1.5 px-4 cursor-pointer">
  ...
  @else
    <div class="flex  justify-center items-center">
        <dotlottie-player src="https://lottie.host/12a488e5-f093-4994-9349-f9d9774b2532/Xk2QIcSlFE.json" background="transparent" speed="1" style="width: 400px; height: 400px;" loop autoplay></dotlottie-player>
       <div class="font-semibold mb-10 lg:text-3xl">
        Image is not on the server <br><span class="text-red-500"> upload the picturer</span>
       </div>
    </div>
    @endif
</div>
