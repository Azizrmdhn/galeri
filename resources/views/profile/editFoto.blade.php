...
<nav class="flex justify-around items-center py-10">
    ...
    <div class="logo flex items-center justify-center gap-5">
        <img src="{{asset('assets/image/wg1.png')}}" width="30" alt="logoPinterest" />
        <h1 class="font-bold text-[15px]">WEBSITE IMAGE</h1>
    </div></a>

    <ul class=" justify-center items-center gap-10  lg:flex">
        <li>
            <a href="/dashboard" class="text-lime-500 font-bold flex justify-center items-center gap-2">
...
<div class="relative min-h-screen flex flex-wrap gap-10  justify-center  py-12 px-4 sm:px-6 lg:px-8  bg-no-repeat bg-cover items-start">
    {{-- card upload image --}}
	<div class="sm:max-w-lg w-full p-10 shadow-md border-2  border-lime-500 bg-white rounded-xl z-10">
        ...
        <form  method="POST" action="{{ route('dataImage.update', $dataImage->fotoID) }}"  enctype="multipart/form-data">
            @csrf
                    <div class="grid grid-cols-1 space-y-2">
                        ...
                        <input class="text-base p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-lime-500" name="judulFoto" type="text" id="judulFoto" value="{{$dataImage->judulFoto}}" required >
                    </div>
                    <div class="grid grid-cols-1 space-y-2">
                        <label class="text-sm font-bold text-gray-500 tracking-wide">Deskripsi gambar</label>
                        <input class="text-base p-2 border border-gray-300 rounded-lg focus:outline-none focus:border-lime-500" name="deskripsiFoto" type="text" value="{{$dataImage->deskripsiFoto}}" required>
                    </div>
                    <div class="grid grid-cols-1 space-y-2">
                                ...
                                <label class="flex flex-col rounded-lg border-4 border-dashed w-full h-60 p-10 group text-center">
                                     <div class="h-full w-full text-center flex flex-col items-center justify-center ">
                                        <div class="py-10 mx-auto">
                                            ...
                                            <h1 class="text-lime-500">{{ $errors->first('lokasiFile') }}</h1>
                                        @else
...
<div class="flex flex-col gap-5">
    ...
    <div>
      <span class="text-white text-xs font-bold rounded-lg bg-lime-500 inline-block mt-4 ml-4 py-1.5 px-4 cursor-pointer">kategori image</span>
      @if ($errors->has('lokasiFile'))
      <h1 class="text-2xl mt-2 ml-4 font-bold text-lime-500 cursor-pointer  transition duration-100">{{ $errors->first('judulFoto') }}</h1>
      @else
    ...
    <div class="z-10 absolute  mb-10 m-2  px-2 bg-lime-500 text-white rounded-full py-2 text-[10px] ">( PNG, JPG/JPEG , SVG )</div>
  ...
  <script src="https://unpkg.com/@lottiefiles/lottie-player@latest/dist/lottie-player.js"></script><lottie-player src="https://lottie.host/17cf870d-e054-4f71-ac52-245d4433974b/w3rw49CaoA.json" background="##FFFFFF" speed="1" style="width: 300px; height: 300px" loop  autoplay direction="1" mode="normal"></lottie-player></div>
</div>
